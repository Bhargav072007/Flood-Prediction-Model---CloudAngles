...
def band_chart_svg(
    ...
    body = [
        f'<text x="60" y="60" fill="#eef7fb" font-size="30" font-family="Segoe UI" font-weight="700">{esc(title)}</text>',
        f'<text x="60" y="92" fill="#9eb7c5" font-size="16" font-family="Segoe UI">{esc(subtitle)}</text>',
        f'<rect x="{left-20}" y="{top-20}" width="{plot_w+40}" height="{plot_h+40}" rx="20" fill="#0f2130" stroke="#27495e"/>',
    ]
    for i in range(5):
        y = top + (plot_h / 4) * i
        body.append(f'<line x1="{left}" y1="{y:.1f}" x2="{right}" y2="{y:.1f}" stroke="#244255"/>')
    body += [
        ...
        f'<text x="{left}" y="{height-28}" fill="#c9d9e2" font-size="12" font-family="Segoe UI">{esc(str(dates.iloc[0])[:10])}</text>',
        f'<text x="{width-145}" y="{height-28}" fill="#c9d9e2" font-size="12" font-family="Segoe UI">{esc(str(dates.iloc[len(dates)-1])[:10])}</text>',
        f'<text x="24" y="{height/2:.1f}" transform="rotate(-90 24 {height/2:.1f})" fill="#c9d9e2" font-size="13" font-family="Segoe UI">{esc(y_label)}</text>',
...
def card_svg(path: Path, title: str, subtitle: str, items: list[tuple[str, str]], accent: str = "#39d0ff") -> None:
    ...
    body = [
        f'<text x="50" y="56" fill="#eef7fb" font-size="30" font-family="Segoe UI" font-weight="700">{esc(title)}</text>',
        f'<text x="50" y="88" fill="#9eb7c5" font-size="16" font-family="Segoe UI">{esc(subtitle)}</text>',
        f'<rect x="40" y="116" width="{width-80}" height="{height-146}" rx="22" fill="#0f2130" stroke="#27495e"/>',
    ]
    y = 155
    for label, value in items:
        body += [
            f'<text x="70" y="{y}" fill="{accent}" font-size="15" font-family="Segoe UI" font-weight="700">{esc(label)}</text>',
            f'<text x="360" y="{y}" fill="#eef7fb" font-size="14" font-family="Segoe UI">{esc(value)}</text>',
            f'<line x1="65" y1="{y+18}" x2="{width-65}" y2="{y+18}" stroke="#244255"/>',
        ]
        y += 40
    write_svg(path, width, height, body)
...
def simple_bar_svg(path: Path, title: str, subtitle: str, data: dict[str, float], color: str) -> None:
    ...
    body = [
        f'<text x="60" y="60" fill="#eef7fb" font-size="30" font-family="Segoe UI" font-weight="700">{esc(title)}</text>',
        f'<text x="60" y="92" fill="#9eb7c5" font-size="16" font-family="Segoe UI">{esc(subtitle)}</text>',
        f'<rect x="50" y="120" width="{width-100}" height="{height-170}" rx="20" fill="#0f2130" stroke="#27495e"/>',
    ]
    y = top + 42
    for k, v in items:
        w = (v / max_v) * 730
        body += [
            f'<text x="{left}" y="{y}" fill="#c9d9e2" font-size="14" font-family="Segoe UI">{esc(k)}</text>',
            f'<rect x="{bar_x}" y="{y-13}" width="760" height="18" rx="9" fill="#173244"/>',
            f'<rect x="{bar_x}" y="{y-13}" width="{w:.1f}" height="18" rx="9" fill="{color}"/>',
            f'<text x="{bar_x+780}" y="{y}" text-anchor="end" fill="#eef7fb" font-size="13" font-family="Segoe UI">{v:.3f}</text>',
        ]
        y += 48
    write_svg(path, width, height, body)
...
def polygon_svg(path: Path, title: str, subtitle: str, coords: list[list[float]]) -> None:
    ...
    body = [
        f'<text x="60" y="60" fill="#eef7fb" font-size="30" font-family="Segoe UI" font-weight="700">{esc(title)}</text>',
        f'<text x="60" y="92" fill="#9eb7c5" font-size="16" font-family="Segoe UI">{esc(subtitle)}</text>',
        '<rect x="60" y="120" width="780" height="430" rx="22" fill="#0f2130" stroke="#27495e"/>',
        f'<polygon points="{pts}" fill="#39d0ff" opacity="0.28" stroke="#43f0c7" stroke-width="3"/>',
    ]
    write_svg(path, width, height, body)
...
def combined_prediction_svg(df: pd.DataFrame) -> None:
    ...
    for x, y, w, h, title, items, color in boxes:
        body += [
            f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="22" fill="#0f2130" stroke="#27495e"/>',
            f'<text x="{x+24}" y="{y+36}" fill="{color}" font-size="20" font-family="Segoe UI" font-weight="700">{esc(title)}</text>',
        ]
        yy = y + 64
        for item in items:
            body.append(f'<text x="{x+26}" y="{yy}" fill="#eef7fb" font-size="14" font-family="Segoe UI">• {esc(item)}</text>')
            yy += 22
    body += [
...
def generate_factor_family_visuals(df: pd.DataFrame) -> None:
    ...
    site_items = []
    row = df.iloc[0]
    for key in ["site_station_nm", "site_drain_area_va", "site_alt_va", "site_dec_lat_va", "site_dec_long_va", "site_huc_cd"]:
        site_items.append((key.replace("site_", "").replace("_", " "), str(row.get(key, ""))))
    card_svg(OUTPUT_DIR / "factor_05_site_metadata.svg", "Factor 5: Site Metadata", "Gauge identity and physical site summary.", site_items, "#39d0ff")
    ts_items = []
    for key in ["tsmeta_parameter_name", "tsmeta_unit_of_measure", "tsmeta_begin", "tsmeta_end", "tsmeta_computation_period_identifier", "tsmeta_monitoring_location_id"]:
        ts_items.append((key.replace("tsmeta_", "").replace("_", " "), str(row.get(key, ""))[:80]))
    card_svg(OUTPUT_DIR / "factor_06_ts_metadata.svg", "Factor 6: Time-Series Metadata", "Coverage, units, and stream metadata for the primary discharge series.", ts_items, "#43f0c7")
    ...
    rating_items = [
        ("file count", str(row.get("rating_curve_file_count", ""))),
        ("file types", str(row.get("rating_curve_file_types", ""))),
        ("item ids", str(row.get("rating_curve_item_ids", ""))[:120]),
    ]
    card_svg(OUTPUT_DIR / "factor_10_rating_curves.svg", "Factor 10: Rating Curves", "USGS rating-curve availability for stage/discharge interpretation.", rating_items, "#ff7f87")
    nasa_items = [
        ("granule count", str(row.get("nasa_imerg_granule_count", ""))),
        ("time start", str(row.get("nasa_imerg_time_start_min", ""))),
        ("time end", str(row.get("nasa_imerg_time_end_max", ""))),
        ("example title", str(row.get("nasa_imerg_first_title", ""))[:85]),
    ]
    card_svg(OUTPUT_DIR / "factor_11_nasa_imerg_metadata.svg", "Factor 11: NASA IMERG Metadata", "Precipitation metadata path prepared for the next model upgrade.", nasa_items, "#39d0ff")
